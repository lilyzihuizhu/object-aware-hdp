...
acc.trad.traintype.glmer <- 
  glmer(correct ~ training_type + (1|split) + (1|test_idx),
        data = results %>% filter(model=="trad"), family = "binomial")
summary(acc.trad.traintype.glmer)

acc.obj.traintype.glmer <- 
  glmer(correct ~ training_type + (1|split) + (1|test_idx),
        data = results %>% filter(model=="obj"), family = "binomial")
summary(acc.obj.traintype.glmer)
```
...
acc.trad.traintype.lmer <- 
  lmer(acc ~ training_type + (1|split), data = subj.acc %>% filter(model=="trad"))
summary(acc.trad.traintype.lmer)

acc.obj.traintype.lmer <- 
  lmer(acc ~ training_type + (1|split), data = subj.acc %>% filter(model=="obj"))
summary(acc.obj.traintype.lmer)
```
